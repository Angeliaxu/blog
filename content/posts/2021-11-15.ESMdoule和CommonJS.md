...
下面的代码是webpack编译模块之后的代码，为了易读，把函数变量名更改了
```
(()=>{
  "use strict"
  <!-- 模块文件名与模块映射,以路径为key -->
...
总结：
1. require引入模块
2. 模块中导出的变量挂载在exports对象下
3. exports对象是require执行的返回结果
4. 外部模块通过require执行的返回结果取得模块内部变量
...
补充：上面说到可webpack加载模块的机制，只涉及到同步加载模块的情况，那么异步加载的机制是什么呢？大致分为两步
1. 动态的生成script标签去加载js
```
            var installedChunks = {
/******/ 			"main": 0
/******/ 		};
...
/******/ 		};
```
2. 调用webpackJsonp 去异步加载的文件中按照模块
```
